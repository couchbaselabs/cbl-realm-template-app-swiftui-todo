...
actor DatabaseService {
    ...
    var taskLiveQueryObserver: (([Item]?) -> Void)?
    ...
    func initializeDatabase(user: User) {
        do {
            ...
            DispatchQueue.main.async {
                app.databaseState = .notInitialized
            }
            ...
            if let db = self.database  {
                ...
                self.taskCollection = try db.createCollection(name: _taskCollectionName, scope:_scopeName)
                if let collection = self.taskCollection {
                    //create index
                    let indexConfig = ValueIndexConfiguration(["ownerId"])
                    try collection.createIndex(withName:"idxTasksOwnerId", config: indexConfig)
                    ...
                    //add authentication
                    let auth = BasicAuthenticator(username: user.username, password: user.password)
    ...
    func addTask(taskSummary: String) {
        do {
            ...
            guard let currentuser = app.currentUser
                ...
                app.error = InvalidCredentialsException(message: "User is not logged in.")
                return
            }
            ...
            let task = Item(isComplete: false, summary: taskSummary, ownerId: currentuser.username)
            if let json = task.toJSON() {
                ...
                app.error = InvalidStateError(message: "item could not be serialized")
            }
    ...
    /// Updates the completion status of a specified task in the database.
    ///
